...
def test_BTH():
    ...
    """
    [maf, n, intercept, beta, sigma2, logalpha, T, c] = [
        0.2,
        300,
        0,
        0.1,
...
def Lev(x, y):
    ...
    sample2 = [
        y[i] for i in range(len(x)) if int(roundedX[i]) != 0 and int(roundedX[i]) != 1
    ]  # y2
    ...
    zdd = 1.0 / N * (np.sum(z0) + np.sum(z1) + np.sum(z2))
    z0d = np.mean(z0)
    z1d = np.mean(z1)
    z2d = np.mean(z2)
    ...
    W2 = 1.0 * (
        np.sum([(z0[j] - z0d) ** 2 for j in range(len(z0))])
        + np.sum([(z1[j] - z1d) ** 2 for j in range(len(z1))])
        + np.sum([(z2[j] - z2d) ** 2 for j in range(len(z2))])
    )
    return W1 / W2
...
def BTH(genotype, phenotype):
    ...
    [numerator, alpha1, inter1, beta1, sigma1] = INLACauchy_log_Laplace(
        genotype, phenotype, theta1, theta2, v, b, 1, gamma
    )
    # compute the denominator of the BF: P (phenotype | genotype,  log alpha = 0)
    [denominator, alpha2, inter2, beta2, sigma2] = INLACauchy_log_Laplace(
        genotype, phenotype, theta1, theta2, v, b, 0, gamma
    )
...
def INLACauchy_log_Laplace(
    genotype, phenotype, theta1, theta2, v, lam, int_type, gamma
):
    """
    ...
    if int_type == 1:
        ...
        bound1 = 1.0  # does not matter for Cauchy
    ...
    if int_type != 0:
        ...
        if int_type == -1:
            ans = optimize.fmin_tnc(
                lambda x: INLACauchy_h(x, params),
                [1.0, 1.0, 0.0],
                fprime=lambda x: INLACauchy_hprime(x, params),
        ...
        evaluate_hess = INLACauchy_h_hess(
            [alpha_hat, sigma_hat, beta0_hat], params
        )  # fing the values of the hessian terms at MAP
        d = 3.0

        S2 = sum(
            [
                (genotype[i] ** 2) * (alpha_hat ** genotype[i])
                for i in range(len(genotype))
            ]
        )
        S1 = sum(
            [genotype[i] * (alpha_hat ** genotype[i]) for i in range(len(genotype))]
        )
        Q1 = sum(
            [
        ...
        beta_hat = (
            1.0 / (v + 1.0 / sigma_hat * S2) * 1.0 / sigma_hat * (Q1 - beta0_hat * S1)
        )
    else:
        ans = optimize.fmin_tnc(
            lambda x: INLACauchy_h(x, params),
        ...
        evaluate_h = INLACauchy_h(
            [sigma_hat, beta0_hat], params
        )  # find the value of the h function at the MAP estimates
        evaluate_hess = INLACauchy_h_hess(
            [sigma_hat, beta0_hat], params
        )  # fing the values of the hessian terms at MAP
        d = 2.0
...
def INLACauchy_h(x, params):
    ...
    if int_type != 0:
        ...
        if int_type == -1:  # will not matter for Cauchy
            prior_on_alpha = np.log(0.5 * 1.0 / abs(lam)) + (1.0 / lam - 1.0) * np.log(
                alpha
            )
        else:
            prior_on_alpha = -np.log(
                np.pi * gamma + np.pi / gamma * ((np.log(alpha)) ** 2)
            ) - np.log(
                alpha
            )  # Cauchy
    ...
    G = sigma * v + S2
    prior_on_sigma = (-theta1 - 1) * np.log(sigma) - theta2 * 1.0 / sigma
    ...
    L0 = (
        (-N / 2.0 + 0.5) * np.log(sigma)
        - 0.5 * np.log(G)
        + 0.5 * np.sum(genotype) * np.log(alpha)
    )
    L1 = -1.0 / (2 * sigma) * (R2 - Q1 * Q1 / G)
    L2 = 1.0 / sigma * beta0 * (R1 - Q1 * S1 / G)
    L3 = -1.0 / (2 * sigma) * beta0 * beta0 * (S0 - S1 * S1 / G)

    h = -1.0 / N * (L0 + L1 + L2 + L3 + prior_on_alpha + prior_on_sigma)
...
def INLACauchy_hprime(x, params):
    ...
    if int_type != 0:
        ...
        if int_type == -1:
            d_alpha_prior_on_alpha = (
                (1.0 / lam - 1.0) * 1.0 / alpha
            )  # not used for Cauchy
        else:
            d_alpha_prior_on_alpha = -(
                gamma**2 + (np.log(alpha)) ** 2 + 2 * np.log(alpha)
            ) / (
                gamma**2 * alpha + alpha * (np.log(alpha)) ** 2
            )  # for Cauchy
    else:
        alpha = 1.0
        sigma = x[0]
    ...
    ### d_sig_L0
    d_sig_L0 = (-0.5 * N + 0.5) * 1.0 / sigma - 0.5 * v / G
    ### d_sig_L1
    d_sig_L1 = 1.0 / (2 * sigma * sigma) * (R2 - Q1 * Q1 / G) - 1.0 / (
        2 * sigma
    ) * v * Q1 * Q1 / (G * G)
    ### d_sig_L2
    d_sig_L2 = -1.0 / (sigma * sigma) * beta0 * (
        R1 - Q1 * S1 / G
    ) + 1.0 / sigma * beta0 * Q1 * S1 * v / (G * G)
    ### d_sig_L3
    d_sig_L3 = 1.0 / (2 * sigma * sigma) * beta0 * beta0 * (S0 - S1 * S1 / G) - 1.0 / (
        2 * sigma
    ) * beta0 * beta0 * S1 * S1 * v / (G * G)
    ### d_sig_prior_on_alpha
    d_sig_prior_on_alpha = 0
    ### d_sig_prior_on_sigma
    d_sig_prior_on_sigma = (-theta1 - 1) * (1.0 / sigma) + theta2 / (sigma * sigma)
    ...
    d_beta0_L1 = 0
    ### d_beta0_L2
    d_beta0_L2 = 1.0 / sigma * (R1 - Q1 * S1 / G)
    ### d_beta0_L3
    d_beta0_L3 = -1.0 / sigma * beta0 * (S0 - S1 * S1 / G)
    ...
    if int_type == 0:
        ...
        ### d_alpha_L0
        d_alpha_L0 = -0.5 * 1.0 / alpha * S3 / G + 0.5 * np.sum(genotype) * 1.0 / alpha
        ### d_alpha_L1
        d_alpha_L1 = (
            -0.5
            * 1.0
        ...
        d_alpha_L2 = (
            ...
            * 1.0
            / alpha
            * (Q1 - Q2 * S1 / G - Q1 * S2 / G + Q1 * S1 * S3 / (G * G))
        )
        ### d_alpha_L3
        d_alpha_L3 = (
            -0.5
...
def INLACauchy_hhprime(x, params):
    ...
    if int_type == 0:
        ...
        if int_type == -1:  # won't get here for cauchy
            daa_prior_on_alpha = -(1.0 / lam - 1) * 1.0 / (alpha * alpha)
        else:
            T1 = gamma**2 * (gamma**2 - 2) + 2 * (gamma**2 + 1) * (
                np.log(alpha) ** 2
            )
            T1 = T1 + 2 * gamma**2 * np.log(alpha) + (np.log(alpha) ** 4)
            T1 = T1 + 2.0 * (np.log(alpha) ** 3)
    ...
    ### dss_L0
    dss_L0 = -(-N / 2.0 + 0.5) * 1.0 / (sigma**2) + 0.5 * (v**2) / (G**2)

    ### dss_L1
    dss_L1 = (
        -1.0 / (sigma**3) * (R2 - Q1 * Q1 / G)
        + 1.0 / (sigma**2) * (v * (Q1**2) / (G**2))
        + 1.0 / sigma * (Q1**2) * (v**2) / (G**3)
    )

    ### dss_L2
    dss_L2 = (
        2.0 / (sigma**3) * beta0 * (R1 - Q1 * S1 / G)
        - beta0 / (sigma**2) * Q1 * S1 * v / (G**2)
        + (-1.0 / (sigma**2)) * beta0 * Q1 * S1 * v / (G**2)
        + 1.0 / sigma * beta0 * (-2) * Q1 * S1 * v * v / (G**3)
    )

    ### dss_L3
    dss_L3 = (
        -1.0 / (sigma**3) * (beta0**2) * (S0 - S1 * S1 / G)
        + 1.0 / (sigma**2) * (beta0**2) * (S1**2) * v / (G**2)
        + +1.0 / sigma * (beta0**2) * (S1**2) * v * v / (G**3)
    )
    ...
    ### dss_prior_on_sigma
    dss_prior_on_sigma = -(-theta1 - 1) * 1.0 / (sigma**2) - 2 * theta2 / (sigma**3)
    ...
    dsb_L1 = 0
    ### dsb_L2
    dsb_L2 = -1.0 / (sigma**2) * (R1 - Q1 * S1 / G) + 1.0 / sigma * Q1 * S1 * v / (
        G**2
    )
    ### dsb_L3
    dsb_L3 = beta0 / (sigma**2) * (S0 - S1**2 / G) - (beta0 / sigma) * (
        S1**2
    ) * v / (G**2)
    ### dsb_prior_on_alpha
    ...
    hss = (
        -1.0
        / N
        * (dss_L0 + dss_L1 + dss_L2 + dss_L3 + dss_prior_on_alpha + dss_prior_on_sigma)
    )
    hsb = (
        -1.0
        / N
        * (dsb_L0 + dsb_L1 + dsb_L2 + dsb_L3 + dsb_prior_on_alpha + dsb_prior_on_sigma)
    )
    hbb = (
        -1.0
        / N
        * (dbb_L0 + dbb_L1 + dbb_L2 + dbb_L3 + dbb_prior_on_alpha + dbb_prior_on_sigma)
    )
    ...
    if int_type == 0:
        ...
        ### daa_L0
        daa_L0 = (
            1.0 / (2 * (alpha**2)) * 1.0 / G * (S3 - S4)
            + 1.0 / (2 * alpha**2) * (S3**2) / (G**2)
            - 1.0 / (2 * alpha**2) * np.sum(genotype)
        )
        ...
        P2 = (
            O2
            - (2 * Q2**2 + 2 * Q1 * Q3) / G
            + (4.0 * Q1 * Q2 * S3 + (Q1**2) * S4) / (G**2)
            - 2.0 * (Q1**2) * (S3**2) / (G**3)
        )
        ##                 ##

        daa_L1 = (
            0.5 * 1.0 / sigma * 1.0 / (alpha**2) * P1
            - 0.5 * 1.0 / sigma * 1.0 / (alpha**2) * P2
        )
        ...
        P2 = (
            Q2
            - (Q3 * S1 + Q2 * S2 + Q2 * S2 + Q1 * S3) / G
            + (S3 * (Q2 * S1 + Q1 * S2)) / (G**2)
            + (Q2 * S1 * S3 + Q1 * S2 * S3 + Q1 * S1 * S4) / (G**2)
            - (2 * Q1 * S1 * (S3**2)) / (G**3)
        )

        daa_L2 = (
            -1.0 / sigma * beta0 * 1.0 / (alpha**2) * P1
            + 1.0 / sigma * beta0 * 1.0 / (alpha**2) * P2
        )
        ...
        P2 = (
            ...
            - 2 * (S2**2 + S1 * S3) / G
            + 2 * S1 * S2 * S3 / (G**2)
            + (2 * S1 * S2 * S3 + S1**2 * S4) / (G**2)
            - (2 * S1**2 * S3**2) / (G**3)
        )

        daa_L3 = (
            1.0 / (2 * sigma) * (beta0**2) / (alpha**2) * P1
            - 1.0 / (2 * sigma) * (beta0**2) / (alpha**2) * P2
        )
        ...
        ################# das #################
        ### das_L0
        das_L0 = 0.5 * 1.0 / alpha * v * S3 / (G**2)

        ### das_L1
        P1 = O1 - 2.0 * Q1 * Q2 / G + (Q1**2) * S3 / (G**2)
        P2 = 2 * Q1 * Q2 * v / (G**2) - 2 * Q1**2 * S3 * v / (G**3)

        das_L1 = (
            0.5 * (1.0 / (sigma**2)) * (1.0 / alpha) * P1
            - (0.5 * 1.0 / sigma) * (1.0 / alpha) * P2
        )
        ...
        ### das_L3
        P1 = 1.0 / alpha * (S1 - 2 * S1 * S2 / G + S1**2 * S3 / (G**2))
        P2 = (
            1.0
            / alpha
            * (2 * S1 * S2 * v / (G**2) - 2 * S3 * (S1**2) * v / (G**3))
        )
        ...
        dab_L2 = (
            1.0
            / (sigma * alpha)
            * (Q1 - Q2 * S1 / G - Q1 * S2 / G + Q1 * S1 * S3 / (G**2))
        )

        ### dab_L3
        dab_L3 = (
            -beta0 / (sigma * alpha) * (S1 - 2 * S1 * S2 / G + S1**2 * S3 / (G**2))
        )
...
def INLACauchy_intermediary_params(x, genotype, phenotype, int_type):
    ...
    if int_type != 0:
        ...
        for i in range(len(genotype)):
            ...
            [Q1, Q2, Q3] = [
                Q1 + (phenotype[i] ** 1) * (genotype[i] ** 1) * c,
                Q2 + (phenotype[i] ** 1) * (genotype[i] ** 2) * c,
                Q3 + (phenotype[i] ** 1) * (genotype[i] ** 3) * c,
            ]
            [O1, O2, O3] = [
                O1 + (phenotype[i] ** 2) * (genotype[i] ** 1) * c,
                O2 + (phenotype[i] ** 2) * (genotype[i] ** 2) * c,
                O3 + (phenotype[i] ** 2) * (genotype[i] ** 3) * c,
            ]
